...
4. Adopt du = 0, dp = 0 type of Dirichlet boundary conditions (deduce accelerlation boundary condition from them).
*/
...
namespace hyme1D_FEM3
{
	...
	// Generated according to the above two bcs
...
#define _dNf2_dxi(xi) (0.5)
	...
	//double step_t, cur_step_t;
	// Substep time increment
	double dt; // change its name to "substep_t" next time
	...
	// Init step
	int init_first_step(void);
	int init_other_step(void);

	// Calculate substep
	int init_first_substep(void);
	int init_other_substep(void);
	int substep(void);
	...
	int init_first_step(void)
	{
		...
		// allocate space for Dirichlet boundary conditions
		// a
		acceleration_num = inc_displacement_num;
		if (acceleration_num)
		{
		...
		{
			acceleration_bc = NULL;
		}
		// pa
		p_acceleration_num = inc_pore_pressure_num;
	...
	int init_first_substep(void)
	{
		// Init constant of this substep
		dt_square = dt * dt;
		gamma_dt = gamma * dt;
		one_minus_gamma_dt = (1.0 - gamma) * dt;
		beta_dt_square = beta * dt_square;
		point_five_minus_beta_dt_square = (0.5 - beta) * dt_square;

		// Calculate Dirichlet boundary conditions and apply initial conditions
		size_t node_id_tmp;
		// Velocity
		...
		{
			node_id_tmp = inc_displacement_bc[i].node_id;
			acceleration_bc[i].node_id = node_id_tmp;
			acceleration_bc[i].a = 2.0 * (inc_displacement_bc[i].du - node_ss[node_id_tmp].v * dt) / dt_square;
			node_ss[node_id_tmp].a = acceleration_bc[i].a;
		}
		// Pore pressure
		...
		{
			node_id_tmp = inc_pore_pressure_bc[i].node_id;
			p_acceleration_bc[i].node_id = node_id_tmp;
			p_acceleration_bc[i].pa = 2.0 * (inc_pore_pressure_bc[i].dp - node_fs[node_id_tmp].pv * dt) / dt_square;
			node_fs[node_id_tmp].pa = p_acceleration_bc[i].pa;
		}
	...
	int init_other_substep(void)
	{
		...
		for (size_t i = 0; i < inc_displacement_num; i++)
		{
			node_id_tmp = acceleration_bc[i].node_id;
			acceleration_bc[i].a = (inc_displacement_bc[i].du - node_ss[node_id_tmp].v * dt - point_five_minus_beta_dt_square * node_ss[node_id_tmp].a * dt_square) / beta_dt_square;
		}
		// Acceleration of pore pressure
		for (size_t i = 0; i < inc_pore_pressure_num; i++)
		{
			node_id_tmp = p_acceleration_bc[i].node_id;
			p_acceleration_bc[i].pa = (inc_pore_pressure_bc[i].dp - node_fs[node_id_tmp].pv * dt - point_five_minus_beta_dt_square * node_fs[node_id_tmp].pa * dt_square) / beta_dt_square;
		}
